...
- **Notify tab Quill rich text editor:** Replaced plain `<textarea>` with Quill 1.3.7 editor (already loaded in dashboard.html). `sendEmailViaApp()` reads `notifyQuill.root.innerHTML` and passes through `_quillHtmlToEmailHtml()` for inline CSS. `loadTemplate()` uses `dangerouslyPasteHTML()`. `saveTemplate()` saves `notifyQuill.root.innerHTML`. Preview renders HTML with merge fields applied. HTML detection in `notify_send` (server-side) skips plain-text conversion when body contains HTML tags.
- **Notify tab error handling:** Wrapped `notify_send` in `_notify_send_inner()` with outer try/except to guarantee JSON responses on unhandled exceptions. JS fetch split into separate try/catch for JSON parse with specific "Server error (status NNN)" toast instead of misleading "Network error".
...
- **Memberships page — editable content via admin:** New "Memberships" tab in admin dashboard (nav button + `page-memberships` div). Fields: Hero Subtext, Section Heading, Body Text (Quill rich text editor). Saved to `DB.pageContent` under keys `mbr-hero-sub`, `mbr-cs-heading`, `mbr-cs-body`. Admin JS: `mbrBodyQuill`, `buildMembershipsPage()`, `saveMembershipsContent()`.
- **Memberships page — Jinja2 smart-quote bug fix:** Template previously used `{{ page_content.get('mbr-cs-heading') or 'Coming Soon' }}` where `'` was a Unicode RIGHT SINGLE QUOTATION MARK (U+2019), causing `jinja2.exceptions.TemplateSyntaxError`. Fix: route pre-computes all values in Python (`mbr_heading`, `mbr_body`, `mbr_hero_sub`) and template uses simple `{{ mbr_heading }}` variables with no string literals inside `{{ }}`. **Never use curly/smart quotes inside Jinja2 `{{ }}` expressions.**
- **Memberships route:** `memberships()` wraps `get_db()` in try/except so a DB failure returns the page with empty content rather than a 500. Passes `canonical_url` and `ga_id`.
- **MS Clarity link in Marketing tab:** Added "MS Clarity ↗" anchor next to Bing Webmaster Tools link (no API integration — Clarity has no public data API).
- **Bing SEO optimization:** IndexNow setup, unique meta descriptions on all pages, intentional noindex on member-only pages, Bing Search Queries widget in admin Marketing tab. Removed debug fields (`_debug_sample_row`, `_debug_row_count`) from Bing search API response.
...
- **DMARC record:** Add TXT record `_dmarc.qbixcentre.com` → `v=DMARC1; p=none; rua=mailto:info@qbixcentre.com` in GoDaddy DNS (improves email deliverability).
- **Duplicate M365 subscription:** A second Microsoft 365 Business Basic subscription was accidentally created. Cancel the duplicate once billing is confirmed stable.
